...
async def song(client, message: Message):
    ...
    if not urlissed:
        ...
        return
    pablo = await client.send_message(message.chat.id, f"**🔎 جاري تحميل : @D8_8Q ** `{urlissed}`")
...
async def vsong(client, message: Message):
    ...
    pablo = await client.send_message(message.chat.id, f"**🔎 يبحث عن : @D8_8Q** `{urlissed}`")
    if not urlissed:
    ...
    try:
        ...
        await event.edit(event, f"**التحميل فشل : @D8_8Q** \n `{str(e)}`")
        return
    c_time = time.time()
